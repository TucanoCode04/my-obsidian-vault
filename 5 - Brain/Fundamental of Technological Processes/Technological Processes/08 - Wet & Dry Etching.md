...
# 08 - Wet & Dry Etching
## Etching
Etching a subtractive process used to remove material from the surface of a substrate. 
...
##### Etchant Properties
The choice of etchant depends on several factors:
- **Etch Rate:** The etch rate is the speed at which the material is removed from the surface. It depends on the concentration of the etchant, the temperature, and the agitation of the solution. Higher concentrations and temperatures increase the etch rate, but can also increase the risk of damaging the substrate or the masking layer. We don't always want to maximize the etch rate, since we want to have good control over the process.
- **Selectivity:** The selectivity selectivity is the ratio between the etch rate of the material to be etched and the etch rate of the masking layer or other materials present on the substrate. High selectivity is desired to ensure that only the desired material is etched, while the masking layer and other materials remain intact. There's normally a trade-off between etch rate and selectivity, higher etch rates can lead to lower selectivity.
- **Isotropy/Anisotropy:** Depending on the application, we may want isotropic or anisotropic etching. Isotropic etching is desired for applications where uniform etching is required, while anisotropic etching is desired for applications where precise control over the etch profile is required. Anisotropy is also described as the ratio between the etch rate of two different crystal planes(crystal plane selectivity).
- **Surface Roughness:** The surface roughness is the measure of the smoothness of the etched surface. Some etchants can create rough surfaces, which can be detrimental to the performance of the device. We want to choose etchants that create smooth surfaces.
- **Control of etch parameters:** The ability to control the etch parameters, such as temperature, concentration, and agitation, is important to ensure reproducibility and consistency of the etching process.
- **Safety of reactants and products:** The safety of the etchant chemicals and the reaction products is important to ensure the safety of the operators and the environment. We want to choose etchants that are safe to handle and dispose of.
- **Cost and availability:** Even the disposal cost of the etchant chemicals and the reaction products is important to consider, especially for large-scale production. We want to choose etchants that are cost-effective and readily available.
- **Capacity for etch stop:** Some etchants can be time controlled, doping controlled, or electrically controlled to stop the etching process at a desired depth or location. This is important for applications where precise control over the etch depth is required.
