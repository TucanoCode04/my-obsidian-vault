...
$\Phi$ is in tur the sum of the external flux $\Phi_{ext}$, coming from the applied magnetic field, and the flux generated by the current flowing in the loop $\Phi_{int}$, when the currents through the two JJs are not equal there will exists a circulating current. We can divide the total flux as: 
- the average current flowing in both junctions $I_{avg} = \frac{(i_1 + i_2)}{2}$. Which, as we saw before, is composed of two symmetric currents flowing in opposite directions around the loop, generating no net flux. So the SQUID output current will be just the sum of the two individual junction currents.
- the circulating current around the contour $I_{circ} = \frac{(i_1 - i_2)}{2}$. This current generates a finite magnetic flux, since it flows in the same direction around the loop. This term is responsible for the flux quantization in the SQUID.
