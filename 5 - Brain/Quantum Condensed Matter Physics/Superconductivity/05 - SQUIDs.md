...
We can see a offset in the maximum current modulation pattern, this is due to the fact that the circulating current generates an internal flux that shifts the effective external flux seen by the SQUID loop. This shift can lead to an asymmetry in the maximum current modulation pattern, causing it to be offset from zero external flux.

Flux quantization takes over the behavior of the SQUID when the loop inductance is significant, leading to a step-like response of the total flux to changes in external flux. 
$$\Phi = \Phi_{ext} + L I_{circ} \approx n \Phi_0 \quad \Rightarrow \quad I_{circ} \approx \frac{(n \Phi_0 - \Phi_{ext})}{L}$$
Meaning that the circulating current adjusts itself to maintain the total flux close to integer multiples of the flux quantum, resulting in a quantized flux state within the SQUID loop.

The circulating current responsible for the maximum current modulation tends to 0 when the loop inductance is very high, because the internal flux generated by any circulating current becomes very large, making it energetically unfavorable for significant circulating currents to flow. As a result, the SQUID behaves more like a single superconducting loop without significant current imbalance between the junctions.
## References
