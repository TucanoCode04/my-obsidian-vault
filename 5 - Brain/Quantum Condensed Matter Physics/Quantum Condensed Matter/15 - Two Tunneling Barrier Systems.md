...
Remember $E_0$ is the resonant energy level in the well, not an actual band (important).
##### SET (Single Electron Transistor)
Using the same principle of resonant tunneling through double barrier structures, we can create a Single Electron Transistor (SET). It basically works by introducing a gate voltage to control the energy levels in the well between the barriers. By adjusting the gate voltage, we can align or misalign the resonant energy levels with the Fermi levels of the source and drain electrodes, thereby controlling the tunneling current through the device.
## References
