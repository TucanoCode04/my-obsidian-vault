...
This expression gives the total transition rate from the initial state to all possible final states that can be reached by absorbing a photon with energy $\hbar \omega$. The joint density of states $g(\hbar \omega)$ plays a crucial role in determining the likelihood of transitions, as it reflects the availability of final states at the specific energy difference corresponding to the photon energy.
...
In the visible light range(optical frequencies), the photon momentum $\hbar \vec{k}$ is much smaller than the typical electron momentum in the crystal. Therefore, we can approximate $\vec{k} \approx 0$ for optical optical transitions.(THIS IS IMPORTANT) Meaning, as we said before, that the electron momentum doesn't change during the interaction with the photon and it has vertical transition in k-space.
...
The excitons exists, basically this binding energy exists, if the interaction binding energy is greater than the thermal energy $k_B T$, phonon energy. At room temperature $k_B T \approx 25 meV$, so materials with exciton binding energies larger than this value can sustain excitons at room temperature.
...
The intuition behind this is that excitons are bound states, so they require less energy to be excited compared to free electron-hole pairs. The presence of excitonic states below the bandgap energy leads to additional absorption features in the optical spectrum of semiconductors.
