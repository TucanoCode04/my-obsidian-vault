...
So now we have to study the matrix:$$T^{(31)} = T^{(32)}(\frac{d}{2}) \cdot T^{(21)}(-\frac{d}{2})$$We also know that $k_1=k_3=\sqrt{\frac{2mE}{\hbar^2}}$ and $k_2=\sqrt{\frac{2m(E-V_0)}{\hbar^2}}$, but by knowing that $V_0 > E$ (since it's a barrier) we can write $k_2 = i \alpha$ with $\alpha = \sqrt{\frac{2m(V_0 - E)}{\hbar^2}}$. So the transfer matrices become:$$T^{(32)}(\frac{d}{2}) = \begin{pmatrix} e^{-ik_3\frac{d}{2}} & 0 \\ 0 & e^{ik_3 \frac{d}{2}} \end{pmatrix} \cdot T^{(32)}(x = 0) \cdot \begin{pmatrix} e^{ik_2 \frac{d}{2}} & 0 \\ 0 & e^{-ik_2 \frac{d}{2}} \end{pmatrix}$$Where:$$T^{(32)}(x = 0) = \frac{1}{2k_3} \begin{pmatrix} k_3 + k_2 & k_3 - k_2 \\ k_3 - k_2 & k_3 + k_2 \end{pmatrix} = \frac{1}{2k_1} \begin{pmatrix} k_1 + i\alpha & k_1 - i\alpha \\ k_1 - i\alpha & k_1 + i\alpha \end{pmatrix}$$And similarly for $T^{(21)}(-\frac{d}{2})$:$$T^{(21)}(-\frac{d}{2}) = \begin{pmatrix} e^{-ik_2(-\frac{d}{2})} & 0 \\ 0 & e^{ik_2(-\frac{d}{2})} \end{pmatrix} \cdot T^{(21)}(x = 0) \cdot \begin{pmatrix} e^{ik_1(-\frac{d}{2})} & 0 \\ 0 & e^{-ik_1(-\frac{d}{2})} \end{pmatrix}$$Where:$$T^{(21)}(x = 0) = \frac{1}{2k_2} \begin{pmatrix} k_2 + k_1 & k_2 - k_1 \\ k_2 - k_1 & k_2 + k_1 \end{pmatrix} = \frac{1}{2i\alpha} \begin{pmatrix} i\alpha + k_1 & i\alpha - k_1 \\ i\alpha - k_1 & i\alpha + k_1 \end{pmatrix}$$After calculating the product $T^{(31)} = T^{(32)}(\frac{d}{2}) \cdot T^{(21)}(-\frac{d}{2})$ and substituting into the transmission coefficient formula $T = \left| \frac{1}{T_{22}^{(31)}} \right|^2$, we obtain the well-known result for the transmission probability through a rectangular potential barrier:$$T = \frac{1}{1 + \frac{V_0^2}{4E(V_0 - E)} \sinh^2\left( \sqrt{\frac{2m(V_0 - E)}{\hbar^2}} d \right)}$$This expression shows that the transmission probability decreases exponentially with the width of the barrier $d$ and depends on the height of the barrier $V_0$ relative to the energy of the electron $E$. The hyperbolic sine function $\sinh$ captures the tunneling behavior, indicating that even when the electron's energy is less than the barrier height, there is still a finite probability of tunneling through the barrier. This result, again, is for $E < V_0$.
Since it's a probability, we can check that $0 \leq T \leq 1$. When $d \to 0$, $\sinh(0) = 0$ so $T \to 1$, meaning that if the barrier is very thin the electron can easily tunnel through it. When $d \to \infty$, $\sinh(\infty) \to \infty$ so $T \to 0$, meaning that if the barrier is very thick the electron cannot tunnel through it.
![[Pasted image 20260118191931.png]]
As show is the graph, the transmission coefficient $T$ in function of $\frac{E}{V_0}$, increasing $E$ increases $T$, meaning that the higher the energy of the electron the more likely it is to tunnel through the barrier. Also increasing $d$ decreases $T$, meaning that the thicker the barrier the less likely it is for the electron to tunnel through it.
To get the point where the function intercepts the vertical asymptote at $x=1$, we can-t simply set $E= V_0$ since in that case the expression is not defined. But we can calculate the limit when $E$ approaches $V_0$ from below:$$\lim_{E \to V_0^-} T = \lim_{E \to V_0^-} \frac{1}{1 + \frac{V_0^2}{4E(V_0 - E)} \sinh^2\left( \sqrt{\frac{2m(V_0 - E)}{\hbar^2}} d \right)} = \frac{1}{1 + \frac{m V_0 d^2}{2 \hbar^2}}$$This result shows that as the energy of the electron approaches the barrier height from below, the transmission probability approaches a finite value that depends on the barrier height $V_0$, the width of the barrier $d$, and the mass of the electron $m$.

For $E> V_0$, the wavevector in region 2 becomes real again, $k_2 = \sqrt{\frac{2m(E - V_0)}{\hbar^2}}$. Repeating the same procedure as before, we find that the transmission probability for an electron with energy greater than the barrier height is given by:$$T = \frac{1}{1 + \frac{V_0^2}{4E(E - V_0)} \sin^2\left( \sqrt{\frac{2m(E - V_0)}{\hbar^2}} d \right)}$$In this case, the transmission probability exhibits oscillatory behavior due to the sine function, indicating that there are certain energies at which the electron can pass through the barrier with high probability, known as resonant tunneling. These resonances occur when the width of the barrier and the energy of the electron satisfy specific conditions that allow for constructive interference of the wavefunction within the barrier region.
![[Pasted image 20260118192901.png]]
